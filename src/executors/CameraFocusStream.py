    ...
    def _apply_control(self, camera):
        if self.focus_mode == "Manual":
            if self.trigger_af:
                ...
                # Move each axis only when its target changes, using timed
                # continuous moves (fast, and roughly at the requested position;
                # the camera's absolute-position API is unreliable/very slow).
                zoom_t = round(float(self.zoom_value), 3)
                focus_t = round(float(self.focus_value), 3)
                if self.bootstrap.get("last_zoom") != zoom_t:
                    camera.set_zoom(self.zoom_value)
                    self.bootstrap["last_zoom"] = zoom_t
                if self.bootstrap.get("last_focus") != focus_t:
